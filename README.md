...
- **Doctor & Patient Module** (Imtiaz Zaman Sami, 23101551): Doctor CRUD, Patient
  CRUD, Search Doctor by Specialty, Doctor List / Doctor Profile / Search Interface
  pages — see [docs/API_ENDPOINTS.md](docs/API_ENDPOINTS.md) for the full route list.
...
Additional demo doctors (different specialties) and patients seeded by the
Doctor & Patient Module all use the same password, `password123`:

| Role | Email | Specialty / Notes |
| --- | --- | --- |
| Doctor | ayesha.rahman@health.test | Neurology |
| Doctor | karim.hossain@health.test | Dermatology |
| Doctor | nusrat.jahan@health.test | Pediatrics |
| Doctor | rafiq.ahmed@health.test | Orthopedics |
| Patient | tanvir.alam@health.test | — |
| Patient | sadia.karim@health.test | — |
...
- `GET /api/doctors`, `GET /api/doctors/{id}`, `POST /api/doctors`, `PUT /api/doctors/{id}`, `DELETE /api/doctors/{id}`
...
- Pages: `GET /doctors` (list), `GET /doctors/{id}` (profile), `GET /doctors/search` (search interface)
